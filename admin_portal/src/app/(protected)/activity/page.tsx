...
const actionTypes = [
  'All', 'Approved', 'Rejected', 'Created', 'Edited', 'Deleted', 'Exported', 'Added', 'Updated', 'Bulk'
];
...
const actionColor = (action: string): string => {
  ...
  if (action.includes('Deactivated')) return 'text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700';
...
export default function ActivityLogsPage() {
  ...
  const admins = ['All', ...Array.from(new Set(mockActivityLogs.map(l => l.admin)))];
    ...
    <div className="space-y-6" style={{ fontFamily: 'Inter, sans-serif' }}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Complete audit trail of all admin actions</p>
        </div>
        <button
          onClick={() => toast.success('Exporting activity logs as CSV...')}
        ...
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none dark:text-white appearance-none min-w-32"
            >
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Total Actions', value: mockActivityLogs.length, icon: <Activity size={16} />, color: '#1E3A8A', bg: '#EFF6FF' },
          { label: 'Today', value: 2, icon: <Calendar size={16} />, color: '#16A34A', bg: '#F0FDF4' },
          { label: 'This Week', value: 8, icon: <Hash size={16} />, color: '#7C3AED', bg: '#F5F3FF' },
          { label: 'Active Admins', value: admins.length - 1, icon: <User size={16} />, color: '#D97706', bg: '#FFFBEB' },
        ].map((s, i) => (
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 overflow-hidden"
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                {['Timestamp', 'Admin', 'Action Taken', 'Target / Form ID'].map(h => (
            ...
            <tbody>
              {filtered.map((log, i) => (
                <tr key={log.id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2.5">
                      <div
                        ...
                        style={{ background: `hsl(${log.admin.charCodeAt(0) * 17 % 360}, 55%, 35%)` }}
                      >
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-600 dark:text-gray-300 font-mono text-xs bg-gray-100 dark:bg-gray-700 px-2.5 py-1 rounded-lg">
                      {log.target}
                    </span>
                  </td>
                </tr>
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Showing <span className="font-medium">{filtered.length}</span> of <span className="font-medium">{mockActivityLogs.length}</span> log entries
          </p>
